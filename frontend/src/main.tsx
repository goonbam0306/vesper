...
import { CalmAuroraEdge } from './components/shell/CalmAuroraEdge'
import { DesignReviewFixture } from './components/shell/DesignReviewFixture'
...
function App() {
  ...
  const designReviewFixture = new URLSearchParams(window.location.search).has('designReviewFixture')
  ...
  if (designReviewFixture) return <DesignReviewFixture />
  if (firstBoot === null) return <main className="setup-shell" data-testid="first-boot-shell"><p>Loading Vesper…</p></main>
  if (!firstBoot) return <main className="setup-shell" data-testid="first-boot-shell"><div className="first-boot-orbit" data-testid="first-boot-orbit" /><section className="setup-card" data-testid="first-boot-panel"><div className="first-boot-brand"><VesperPresenceIcon state="idle" decorative size={24} /><span>VESPER</span></div>
