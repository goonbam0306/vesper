...
function App() {
  ...
  return <div className="shell">
    ...
    <main className="canvas"><header className="topbar"><form onSubmit={search} className="omnibar"><span>⌕</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search…" /><kbd>⌘ K</kbd></form><div className="indicators"><span>{approvals.filter(a => a.decision === 'PENDING').length} approvals</span><span>{tasks.filter(t => t.status !== 'DONE').length} open</span></div></header><section className="content"><div className="eyebrow">{view === 'home' ? 'ATTENTION / TODAY' : view.toUpperCase()}</div>
      ...
      {view === 'processes' && page('Processes', <>{processes.length ? processes.map(process => <div className="resource-card" key={process.process_id}><div><strong>{process.process_id}</strong><p>{process.status} · waiting: {process.waiting_reason || 'none'} · parent: {process.parent_id || 'none'} · dependencies: {process.dependency_state}</p><small>result: {process.result_summary || 'none'} · effect: {JSON.stringify(process.effect_summary || {})}</small></div><VesperPresenceIcon state={processPresenceState(process.status)} size={16} /><span className="tag">{process.status}</span></div>) : <><p className="muted">No durable processes yet.</p><div data-testid="empty-processes">No durable processes yet.</div></>}</>)}
