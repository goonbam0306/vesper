...
function App() {
  ...
  const load = async () => {
    const [p, t, c, i, proc, a, con, s, today, effectsResult, memories] = await Promise.all([
      json('/api/projects'), json('/api/tasks'), json('/api/calendar'), json('/api/ideas'), json('/api/processes'), json('/api/approvals'), json('/api/connections'), json('/api/settings'), json('/api/dashboard/today'), json('/api/effects'), json('/api/memories'),
    ])
  ...
  return <div className="shell">
    <aside className="rail"><div className="brand"><span className="brand-mark">V</span><span>VESPER</span></div><nav>{([['home', 'Home'], ['projects', 'Projects'], ['tasks', 'Tasks'], ['calendar', 'Calendar'], ['ideas', 'Ideas'], ['processes', 'Processes'], ['observability', 'Observability'], ['memory', 'Memory'], ['approvals', 'Approvals'], ['connections', 'Connections'], ['settings', 'Settings']] as [View, string][]).map(([id, label]) => <button className={view === id ? 'active' : ''} onClick={() => nav(id)} key={id}>{label}</button>)}</nav><div className="rail-foot"><span className="pulse" /> Kernel local · ready</div></aside>
    <main className="canvas"><header className="topbar"><form onSubmit={search} className="omnibar"><span>⌕</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search…" /><kbd>⌘ K</kbd></form><div className="indicators"><span>{approvals.filter(a => a.decision === 'PENDING').length} approvals</span><span>{tasks.filter(t => t.status !== 'DONE').length} open</span></div></header><section className="content"><div className="eyebrow">{view === 'home' ? 'ATTENTION / TODAY' : view.toUpperCase()}</div>
      ...
      {view === 'processes' && page('Processes', <>{processes.length ? processes.map(process => <div className="resource-card" key={process.process_id}><div><strong>{process.process_id}</strong><p>{process.status} · waiting: {process.waiting_reason || 'none'} · parent: {process.parent_id || 'none'} · dependencies: {process.dependency_state}</p><small>result: {process.result_summary || 'none'} · effect: {JSON.stringify(process.effect_summary || {})}</small></div><span className="tag">{process.status}</span></div>) : <><p className="muted">No durable processes yet.</p><div data-testid="empty-processes">No durable processes yet.</div></>}</>)}
      ...
      <section className="persistent"><div className="panel-head"><strong>Ask V.</strong><button type="button" className="quiet" onClick={newAskConversation}>New conversation</button></div><div className="ask-transcript" ref={askTranscriptRef} aria-live="polite">{askMessages.map(message => <div className={`ask-turn ${message.role.toLowerCase()}`} key={message.message_id}><strong>{message.role === 'USER' ? 'You' : message.role === 'ASSISTANT' ? 'Vesper' : 'Status'}</strong><p>{message.content}</p></div>)}{askState === 'sending' && <div className="ask-turn system_status"><p>Vesper is thinking…</p></div>}</div>{askState === 'response' && <span role="status" className="sr-only">VESPER_READY</span>}<form onSubmit={submitAsk}><div><span className="spark">✦</span><textarea aria-label="Ask Vesper" value={askPrompt} onChange={e => setAskPrompt(e.target.value)} onKeyDown={handleAskKeyDown} onCompositionStart={() => { askComposing.current = true }} onCompositionEnd={() => { askComposing.current = false }} placeholder="Ask Vesper, or leave a note…" rows={1} /><button type="submit" disabled={askState === 'sending' || !askPrompt.trim()}>{askState === 'sending' ? 'Sending…' : 'Send'}</button></div></form>{askState === 'failure' && <p role="alert">{askError}{askError === 'AI connection is not configured' && <> <button type="button" onClick={() => nav('connections')}>Set up AI</button></>}</p>}<small>Persistent surface · Enter to send · Shift+Enter for new line</small></section>{notice && <div className="notice" onClick={() => setNotice('')}>{notice}</div>}</section></main></div>
}
